...
type Buffers={geo:WebGLBuffer;color:WebGLBuffer;size:WebGLBuffer};
...
void main(){
  ...
  gl_PointSize=min(18.0,aSize*uDpr*sqrt(max(uZoom,.35)));
  vColor=aColor;
  vAlpha=clamp(.34+uZoom*.16,.42,.94);
}`;
...
export function createNetworkPointRenderer(canvas:HTMLCanvasElement,colors:Record<string,string>){
  ...
  const geo=gl.createBuffer(),color=gl.createBuffer(),size=gl.createBuffer();
  if(!geo||!color||!size){gl.deleteProgram(program);throw new Error('Unable to allocate network GPU buffers.')}
  const buffers:Buffers={geo,color,size};
  const geoLocation=gl.getAttribLocation(program,'aGeo'),colorLocation=gl.getAttribLocation(program,'aColor'),sizeLocation=gl.getAttribLocation(program,'aSize');
  const zoomLocation=gl.getUniformLocation(program,'uZoom'),panLocation=gl.getUniformLocation(program,'uPan'),viewportLocation=gl.getUniformLocation(program,'uViewport'),dprLocation=gl.getUniformLocation(program,'uDpr');

  gl.useProgram(program);
  gl.enable(gl.BLEND);gl.blendFunc(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA);gl.disable(gl.DEPTH_TEST);
  let pointCount=0,lastPoints:readonly NetworkRenderPoint[]|null=null,lastMode='';
  ...
  const setData=(points:readonly NetworkRenderPoint[],mode:'coordinates'|'aggregate')=>{
    ...
    lastPoints=points;lastMode=mode;pointCount=points.length;
    const geoData=new Float32Array(pointCount*2),colorData=new Float32Array(pointCount*3),sizeData=new Float32Array(pointCount);
    ...
    gl.useProgram(program);bind(buffers.geo,geoLocation,2,geoData);bind(buffers.color,colorLocation,3,colorData);bind(buffers.size,sizeLocation,1,sizeData);
  };
  ...
  const render=(width:number,height:number,dpr:number,zoom:number,panX:number,panY:number)=>{
    ...
    gl.uniform1f(zoomLocation,zoom);gl.uniform2f(panLocation,panX,panY);gl.uniform2f(viewportLocation,width,height);gl.uniform1f(dprLocation,dpr);
  ...
  return {
    setData,
    render,
    dispose(){gl.deleteBuffer(buffers.geo);gl.deleteBuffer(buffers.color);gl.deleteBuffer(buffers.size);gl.deleteProgram(program)},
  };
}
