...
const POS = [[-4.45,1.95,-1.3],[4.35,1.85,-2.5],[4.55,-1.5,-1.2],[-4.45,-1.55,-2.25],[0,-2.75,.45]] as const;
...
function addDrone(scene:THREE.Scene){
  ...
  drone.position.set(2.0,3.35,-3.2);
...
export default function PortalOrbitalNav({portals,agreementOnly=false,onEnter}:{portals:readonly Portal[];agreementOnly?:boolean;onEnter?:()=>void}){
  ...
  useEffect(()=>{
    ...
    const nodes:PortalNode[]=shown.map((portal,i)=>{
      const color=COLORS[portal.tone]||0x78e8ff;const group=new THREE.Group();
      const position=agreementOnly?[0,.15,-1.1]:POS[i];group.position.set(position[0],position[1],position[2]);
      ...
      ring.userData={index:i,href:portal.href};inner.userData={index:i,href:portal.href};
      scene.add(group);return{group,ring,inner,shader,portal,index:i};
    });
    ...
    let travelIndex=-1,travelStart=0,travelFrom=new THREE.Vector3(),travelTo=new THREE.Vector3(),navigationTimer=0;
    ...
    const click=()=>{
      ray.setFromCamera(pointer,camera);const hit=ray.intersectObjects(interactive)[0];if(!hit||travelIndex>=0)return;
      const index=hit.object.userData.index as number;const node=nodes[index];if(!node)return;
      travelIndex=index;travelStart=performance.now();travelFrom=camera.position.clone();travelTo=node.group.position.clone().add(new THREE.Vector3(0,0,.28));setActive(index);
    ...
    const loop=(now:number)=>{
      ...
      ray.setFromCamera(pointer,camera);const hit=travelIndex<0?ray.intersectObjects(interactive)[0]:undefined;hovered=hit?(hit.object.userData.index as number):-1;setActive(current=>current===hovered?current:hovered);

      nodes.forEach((node,i)=>{
        const isHot=i===hovered||i===travelIndex;const wanted=isHot?1.18:1;
        ...
        node.ring.rotation.z=Math.sin(t*.55+i)*.07;node.inner.rotation.z=t*(i%2?.045:-.038);
      });

      astronaut.rotation.y=Math.sin(t*.42)*.10+pointer.x*.035;astronaut.position.y=-.35+Math.sin(t*.8)*.035;
      drone.rotation.x=Math.sin(t*.38)*.16;drone.rotation.y=t*.22;drone.position.y=3.35+Math.sin(t*.7)*.14;
      stars.rotation.y=t*.003;halo.rotation.z=t*.025;

      if(travelIndex>=0){
        const p=ease((now-travelStart)/880);camera.position.lerpVectors(travelFrom,travelTo,p);camera.fov=47-p*19;camera.updateProjectionMatrix();camera.lookAt(nodes[travelIndex].group.position);
      }else{
        targetX+=(pointer.x*.42-targetX)*.025;targetY+=(.6+pointer.y*.2-targetY)*.025;
      ...
      setLabels(nodes.map(node=>{const p=node.group.position.clone().project(camera);return{x:(p.x*.5+.5)*host.clientWidth,y:(-.5*p.y+.5)*host.clientHeight,visible:p.z<1}}));
      renderer.render(scene,camera);
    };
    loop(performance.now());

    return()=>{
      cancelAnimationFrame(raf);if(navigationTimer)window.clearTimeout(navigationTimer);removeEventListener('resize',resize);renderer.domElement.removeEventListener('pointermove',move);renderer.domElement.removeEventListener('click',click);
      scene.traverse(object=>{const mesh=object as THREE.Mesh;if(mesh.geometry)mesh.geometry.dispose();const material=mesh.material;if(Array.isArray(material))material.forEach(item=>item.dispose());else material?.dispose()});
      starsGeo.dispose();renderer.dispose();if(renderer.domElement.parentNode===host)host.removeChild(renderer.domElement);
    };
  },[agreementOnly,onEnter,portals,router]);

  const shown=agreementOnly?portals.slice(-1):portals;
  return <nav className={styles.root} aria-label="Spatial provider portals"><div ref={mount} className={styles.canvas}/><div className={styles.labels}>{shown.map((portal,i)=><a key={portal.id} href={portal.href} data-active={active===i} style={{left:labels[i]?.x,top:labels[i]?.y,opacity:labels[i]?.visible?1:0}}><small>{portal.number} / PORTAL</small><strong>{portal.title}</strong><span>{portal.note}</span></a>)}</div><p className={styles.hint}>MOVE TO FOCUS · SELECT A PORTAL TO TRAVEL</p></nav>;
}
