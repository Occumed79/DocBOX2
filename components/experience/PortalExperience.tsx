...
export function AgreementPortal(){const [specialty,setSpecialty]=useState('Occupational Medicine');useEffect(()=>{const requested=new URLSearchParams(window.location.search).get('specialty');if(requested&&AGREEMENT_SERVICES[requested])setSpecialty(requested)},[]);return <PortalShell active="agreement" eyebrow="PORTAL 05 / PROVIDER AGREEMENT" title="Define the relationship." intro="Prepare facility information, services, and pricing for Network Management review."><div className={styles.agreementIntro}><span className={styles.kicker}>OCCU-MED FORMS / {specialty.toUpperCase()}</span><h2>Provider Fee Proposal</h2><p>This open proposal does not execute a final service agreement. Accepted pricing proceeds to the secure Occu-Med Forms invitation workflow.</p></div><Suspense fallback={<p>Preparing agreement…</p>}><PricingAgreementBuilder specialty={specialty} services={AGREEMENT_SERVICES[specialty]}/></Suspense></PortalShell>}
