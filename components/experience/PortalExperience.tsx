...
const QUESTIONS=[['What is Occu-Med’s role?','Occu-Med coordinates the referral, scheduling, documentation follow-up, quality assurance, and applicable medical review.'],['What does our clinic receive?','The clinic receives an authorization identifying the examinee, requested services, required forms, and return instructions.'],['Should we perform services outside the authorization?','No. Contact Occu-Med for approval before providing any service that is not listed on the authorization.'],['What must be returned?','Return every requested form, report, tracing, image, laboratory result, and vaccination record required by the case.'],['Does our provider make the employment decision?','No. The clinic documents its findings. Occu-Med evaluates those findings against the applicable job or deployment requirements.'],['How are missing records handled?','Provider Relations follows up with the clinic, and Quality Assurance identifies incomplete, missing, or inconsistent documentation.'],['How should we invoice?','Invoice Occu-Med using the accepted fee schedule and the billing directions provided with the referral.'],['Can multiple locations participate?','Yes. Each location and its available services can be identified in the provider proposal and reviewed by Network Management.']];
